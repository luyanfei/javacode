...
import cn.jhc.shared.PhotoNotFoundException;

import com.google.gwt.user.server.rpc.RemoteServiceServlet;

@WebServlet(urlPatterns={"/photomvp2/service"})
public class PhotoAlbumServiceImple extends RemoteServiceServlet implements
		PhotoAlbumService {
	...
	/**
	 * Just pretend the update is successful by returning the passed in
	 * PhotoDetails object
	 */
	...
	public Vector<PhotoDetails> getPhotoList(int start, int length) {
		...
		Iterator<PhotoDetails> i = data.iterator();
		int pos = 0;
		while (i.hasNext() && pos++ < start)
			i.next();
		while (i.hasNext() && pos++ < (start + length))
			toReturn.add(i.next());
		return toReturn;
	}
